...
export default function SubMenuItem({ url = '', text = '', level = 1, children, onClick }) {
  ...
  return (
    <li className={groupClass}>
      <Link
        to={url}
        className="block flex items-center justify-between border-b-1 border-pearl-bush px-5 py-5 text-shark transition-colors duration-150 ease-in-out lg:border-pampas lg:py-3.5 lg:hover:bg-pampas lg:focus:bg-pampas"
        onClick={(e) => {
          if (onClick) onClick(e);
          e.currentTarget.blur(); // stop drop down from remaining after click
        }}
      >
        {text}
        {children && (
          <svg
            ...
            viewBox="0 0 384 512"
            className={`h-5 w-5 fill-boulder lg:h-2.5 lg:w-2.5 lg:-rotate-90 lg:fill-shark ${mobileSubMenuClickClass}`}
            onClick={(e) => {
              e.preventDefault();
